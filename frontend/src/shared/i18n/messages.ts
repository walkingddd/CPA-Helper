...
const serverMessagePatterns: ServerMessagePattern[] = [
  ...
  [/^API KEY 不能包含首尾空白字符$/, () => 'API key cannot contain leading or trailing whitespace'],
  [/^API KEY 不能超过 400 字节$/, () => 'API key cannot exceed 400 bytes'],
  [/^API KEY 不能包含空白或控制字符$/, () => 'API key cannot contain whitespace or control characters'],
  [/^API KEY 已存在，不能重复创建$/, () => 'API key already exists and cannot be created again'],
  [/^API KEY 已存在于 CPA，请由管理员使用绑定功能关联用户$/, () => 'API key already exists in CPA. Ask an administrator to bind it to a user.'],
  [/^CPA API KEY 同步状态无法确认，请刷新后检查$/, () => 'CPA API key sync status could not be confirmed. Refresh and verify the key before retrying.'],
